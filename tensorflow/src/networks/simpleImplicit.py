...
class ImplicitLayer(keras.layers.Layer):
    units: int
    input_dim: int
    ...
    def __init__(self, input_dim=32, units=32, batch_size=32, epsilon=0.1, dt=0.1, name="RelaxationLayerImplicit",
                 **kwargs):
        super(ImplicitLayer, self).__init__(**kwargs)

        self.units = units
        self.input_dim = input_dim

        self.A = self.add_weight(shape=(input_dim, units), initializer="random_normal", trainable=True)
